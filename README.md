...
- 오늘 적은 내역은 직접 수정·삭제 (지난 것은 관리자에게 요청)

**관리자**
- 전 직원 매출 · 출금 · 미출금 현황
- 목록 정렬 · 차종별 필터 · 검색 · 체크박스로 골라 합계 보기
- 직원별 상세 내역 — 금액·건수·결제구분을 날짜 제한 없이 바로잡음
- 누가 언제 무엇을 고쳤는지 수정 이력, 삭제된 내역까지 보관
...
**AI (Gemini)**
- **공지 다듬기** — 맞춤법·띄어쓰기를 고치고, 중요한 문단을 소제목 / 꼭 지킬 것 / 강조 / 일반 4단계로 표시. 고친 내용을 먼저 보여주고 관리자가 승인
- **오늘 목표** — 전 직원 평균과 본인 최근 흐름을 견줘 오늘 매출 목표와 실천 팁 제안
- **마감 보고** — 그날 메모·지출·운행시간을 읽고 잘한 점과 개선점을 정리
...
AI 결과는 **만들 때 한 번만 호출하고 DB에 저장**합니다. 공지 서식은 `notices.blocks`에, 코칭과 마감 보고는 `ai_reports`에 (직원 1명 × 하루 1건) 들어갑니다. 직원이 몇 번을 열어봐도 추가 호출이 없습니다.

Gemini 3.x는 기본적으로 생각을 길게 합니다. 실측해 보니 849토큰짜리 공지를 만드는 데 사고에만 6,000토큰을 썼습니다. 그래서 작업마다 `thinkingBudget`을 따로 잡았습니다 — 서식 매기기처럼 판단이 단순한 작업은 조이고, 마감 보고처럼 따져볼 게 많은 작업만 넉넉히 줍니다.

프롬프트에는 **주어진 숫자만 쓰고 없는 값을 지어내지 말 것**을 못 박았습니다. 지출이나 운행시간을 안 적은 날은 연비·시간당 효율을 계산하지 않고, 대신 "적으면 이런 것까지 봐 드릴 수 있습니다"라고만 안내합니다.

말투는 공통 규칙으로 묶었습니다. 사내에서는 서로 형·형님이라 부르지만 공지와 리포트는 글로 남는 것이라 **호칭 없이** 정중한 존댓말로 씁니다.
...
정산은 돈이 걸린 기록이라 두 가지를 지켰습니다.

- **직원은 오늘 적은 것만** 고칠 수 있습니다. 기준을 `work_date`가 아니라 적은 날(`created_at`)로 잡은 이유는, 지난 날짜를 뒤늦게 입력하는 일이 있어 방금 적은 건 바로 고칠 수 있어야 하기 때문입니다. **관리자는 제한이 없습니다** — 직원이 잘못 올린 걸 바로잡는 게 관리자의 일이라서요.
- 이 규칙은 서버 코드와 **RLS 양쪽에** 걸려 있습니다. anon 키는 공개되어 있으므로 화면 코드만 믿으면 직접 호출로 우회할 수 있습니다.

수정·삭제 이력은 화면 코드가 아니라 **DB 트리거**가 남깁니다. 어느 경로로 고치든 빠짐없이 기록되고, 실제로 값이 달라졌을 때만 남습니다. 삭제된 내역도 관리자 화면에서 볼 수 있습니다.
...
│  ├─ gemini.ts       Gemini 호출 · 공통 말투 규칙
│  ├─ notice-blocks.ts 공지 강조 4단계
│  ├─ settlement.ts   사납금 · 실수령 계산
