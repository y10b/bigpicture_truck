...
|---|---|
| 매출 | 신용 + 착불 + 추가금 |
| 사납금 | 그 주에 일한 **앞 5일** × 단가(기본 10만원) — 평일·주말 무관 |
| 실수령 | 매출 − 사납금 |
| 미출금 | 실수령 − 출금액 |
| 주 단위 | **월요일 시작 ~ 일요일 종료** |

사납금은 요일이 아니라 **그 주에 일한 순서**로 붙습니다. 월~금 5일을 채우고 토요일에 더 나오면 그 토요일은 면제이고, 평일 하루를 쉬고 토요일에 대신 나오면 그 토요일에는 사납금이 붙습니다. 단가와 주당 일수는 관리자가 바꿀 수 있습니다.

기간을 주 중간부터 조회해도 순번이 틀어지지 않도록, 사납금 판정은 항상 **그 기간이 걸친 주 전체**를 읽어서 계산합니다.
